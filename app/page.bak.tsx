...
export default function Page(){
 const [user,setUser]=useState<any>(null); const [u,setU]=useState('admin'); const [p,setP]=useState('admin123');
 ...
 useEffect(()=>{const s=localStorage.getItem('currentUser'); if(s) setUser(JSON.parse(s)); ensureDefaultUsers();},[]);
 const allowedCenters=useMemo(()=> user?.role==='center'?centers.filter(c=>c.id===user.centerId):centers,[user]);
 ...
 const saveUser=async()=>{
  ...
  const payload:any={
   ...
   role,
   centerId,
   centerName:centerId?(centers.find(c=>c.id===centerId)?.name||''):'جميع المراكز',
   ...
   if(!found){
    ...
    if(found){
     await setDoc(doc(db,'users',found.username),{
      ...found,
      centerName:found.centerId?(centers.find(c=>c.id===found.centerId)?.name||''):'جميع المراكز',
 ...
 const saveCurrentForm=async()=>{
  ...
  const meta={savedAt:now,updatedAt:now,savedBy:user?.name||'',role:roleTitle(user?.role),centerName:centers.find(c=>c.id===centerId)?.name||'',formTitle:mod.title,completion:currentPct,fieldsCount:count,totalFields:currentFields.length};
  ...
  try{
   ...
   await addAudit('حفظ بيانات نموذج',{centerId,centerName:centers.find(c=>c.id===centerId)?.name||'',moduleId,moduleTitle:mod.title,fieldsCount:count,completion:currentPct});
   await loadAuditRows();
 ...
 async function exportModule(single=true){
  ...
  if(single){const blob=await exportOne(mod,centerId); saveAs(blob,`${fileSafeName(mod.title)}-${fileSafeName(centers.find(c=>c.id===centerId)?.name||'مركز')}-${cleanYearInput(year)}-شهر-${month}.xlsx`); showToast('تم تصدير النموذج بنجاح')}
  ...
  <main className="layout"><aside><div className="sideTitle"><b>القائمة الرئيسية</b><small>النطاق: {user.role==='center'?centers.find(c=>c.id===user.centerId)?.name:'جميع المراكز'}</small></div><button className={'navBtn '+(tab==='dashboard'?'active':'')} onClick={()=>setTab('dashboard')}>🏠 الرئيسية</button><button className={'navBtn '+(tab==='followup'?'active':'')} onClick={()=>setTab('followup')}>📌 متابعة المراكز</button><button className={'navBtn '+(tab==='report'?'active':'')} onClick={()=>setTab('report')}>📄 التقرير التنفيذي</button><button className={'navBtn '+(tab==='entry'?'active':'')} onClick={()=>setTab('entry')}>📊 الإحصائية الشهرية</button><button className={'navBtn '+(tab==='exports'?'active':'')} onClick={()=>setTab('exports')}>📁 التصدير</button>{(user.role==='admin'||user.role==='executive')&&<button className={'navBtn '+(tab==='audit'?'active':'')} onClick={()=>{setTab('audit');loadAuditRows()}}>🧾 سجل العمليات</button>}{user.role==='admin'&&<button className={'navBtn '+(tab==='users'?'active':'')} onClick={()=>{setTab('users');loadUserRows()}}>⚙️ المستخدمون والصلاحيات</button>}<hr/><p className="muted sideSection">النماذج المعتمدة</p>{modules.map(m=><button key={m.id} className={'navBtn moduleBtn '+(moduleId===m.id&&tab==='entry'?'active':'')} onClick={()=>{setModuleId(m.id);setTab('entry')}}><span>{m.title}</span><small>{moduleFilled(m,centerId)}/{moduleTotal(m,centerId)}</small></button>)}</aside>
   <section className="content">
    {tab==='dashboard'&&<div className="pageStack"><div className="welcomeCard executiveHero"><div><span className="officialTag dark">مركز القيادة التنفيذي</span><h2>مركز القيادة التنفيذي للإحصائية الشهرية</h2><p>لوحة تنفيذية موحدة لمتابعة حالة المراكز، المؤشرات الصحية، التنبيهات، والتقارير الشهرية.</p><div className="heroButtons"><button className="btn btnPrimary" onClick={()=>setTab('report')}>عرض التقرير التنفيذي</button><button className="btn btnGhost" onClick={()=>setTab('exports')}>التصدير الشهري</button></div></div><div className="statusPill"><strong>{globalPct}%</strong><span>نسبة الإنجاز العامة</span></div></div>
     ...
     <div className="card"><div className="sectionHead"><div><h2>جاهزية النماذج للمركز المحدد</h2><p className="muted">اضغط على أي نموذج للانتقال مباشرة إلى صفحة الإدخال.</p></div><span className="badge">{centers.find(c=>c.id===centerId)?.name}</span></div><div className="moduleCards moduleCardsGrid">{modules.map(m=>{const f=formCompletion(m,centerId);return <button key={m.id} className="miniModule" onClick={()=>{setModuleId(m.id);setTab('entry')}}><span>{m.title}</span><b>{f.done}/{f.total}</b><em>{f.pct}%</em></button>})}</div></div>
    </div>}
